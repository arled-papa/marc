...
value: A list of the generated rx_rate values for each agent in the agent list
"""
toPlot = {}
...
for users in list_users:
    # For each user create a list for the average packet_rate_rx values for each agent
    toPlot[users] = []
    for agents in list_agents:
        ...
        resultsBytes = []
        # The measurement files from 20 to 29 contain measurements for packet_rate_rx
        for run in range(20, 20 + number_runs):
            ...
            with open("../{}/packet_rate_rx/agent-{}-users-{}-{}".format(input_folder, agents, users, run)) as fp:
                for line in fp:
                    t = (line.split(" "))
                    bytesCnt = bytesCnt + 1
                    if bytesCnt % 2 != 0:
...
plt.savefig("{}/{}_packet_rate_rx_{}.pdf".format(output_folder, controller, out_format))
plt.show()
