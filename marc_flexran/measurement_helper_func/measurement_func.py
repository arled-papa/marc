...
Args:
number_agents: The number of initiated FlexRAN agents
number_users: The number of users per each initiated FlexRAN agent
...
def utilization_report(number_agents, number_users, cpu_utilization_data, mem_utilization_data, round_no):
    ...
    with SCPClient(ssh.get_transport()) as scp:
        scp.get("~/marc/measurement_tools/cpu_utilization", "reports/flexran_cpu_utilization")
        scp.get("~/marc/measurement_tools/mem_utilization", "reports/flexran_mem_utilization")
        scp.get("~/marc/measurement_tools/packet_rate_tx", "flexran_measurements/packet_rate_tx/"
                                                           "agent-{}-users-{}-{}".format(number_agents, number_users,
                                                                                         round_no))
        scp.get("~/marc/measurement_tools/packet_rate_rx", "flexran_measurements/packet_rate_rx/"
                                                           "agent-{}-users-{}-{}".format(number_agents, number_users,
                                                                                         round_no))

    cpu_utilization = list(line.rstrip() for line in open("reports/flexran_cpu_utilization"))
    mem_utilization = list(line.rstrip() for line in open("reports/flexran_mem_utilization"))

    cpu_df = pd.DataFrame(cpu_utilization)
    mem_df = pd.DataFrame(mem_utilization)

    # Store the data to the respective folder
    file_name = "flexran_measurements/agent_{}/Agents_{}_Users_{}_{}" \
        .format(number_agents, number_agents, number_users, round_no)
    mem_file_name = "flexran_measurements/mem_agent_{}/Agents_{}_Users_{}_{}" \
        .format(number_agents, number_agents, number_users, round_no)
