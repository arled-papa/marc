...
import marc_5gempower.measurement_helper_func.controller_connection as connect_controller
...
"""
Function that initiates the benchmarking of 5G-EmPOWER.
...
Args: 
list_agents: The list of 5G-EmPOWER agents to initiate for the benchmark
...
def run_benchmark_5gempower(args=None):
    ...
    if not os.path.isdir("5gempower_measurements/packet_rate_rx"):
        os.mkdir("5gempower_measurements/packet_rate_rx")

    if not os.path.isdir("5gempower_measurements/packet_rate_tx"):
        os.mkdir("5gempower_measurements/packet_rate_tx")

    for agent in list_agents:
        if not os.path.isdir("5gempower_measurements/agent_{}".format(agent)):
            os.mkdir("5gempower_measurements/agent_{}".format(agent))
        if not os.path.isdir("5gempower_measurements/mem_agent_{}".format(agent)):
            os.mkdir("5gempower_measurements/mem_agent_{}".format(agent))
        for users in list_users:
            # Number of runs per each configuration
            runs = 3
            for run in range(runs):
                # Start the 5G-EmPOWER controller
                connect_controller.connect_5gempower(controller_ip, "start")
                time.sleep(10)
                # Start cpu consumption measurements
                connect_controller.connect_5gempower(controller_ip, "cpu_stats")
                # Start memory consumption measurements
                connect_controller.connect_5gempower(controller_ip, "mem_stats")
                # Start controller sent packet measurements
                connect_controller.connect_5gempower(controller_ip, "packet_tx", agent, users, run)
                # Start controller received packet measurements
                connect_controller.connect_5gempower(controller_ip, "packet_rx", agent, users, run)
                ...
                # Stop the controller
                connect_controller.connect_5gempower(controller_ip, "stop")
                # Terminate all measurement scripts
                connect_controller.connect_5gempower(controller_ip, "terminate_all_scripts")
